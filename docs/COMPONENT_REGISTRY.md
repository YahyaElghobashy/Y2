...
| /travels/[tripId]/site/[[...path]] | ✅ | `app/(main)/travels/[tripId]/site/[[...path]]/route.ts` | Auth-gated static-file server (nodejs, force-dynamic) for hosted-trip bundles under `content/trips/<hosted_path>/`. 401 no-session, 404 not-found/not-hosted, path-traversal guard, per-bundle default doc (the bundle's own `*.dc.html`, else `index.html` — cambridge-london→`UK Trip.dc.html`, malaysia-vietnam→`Malaysia Trip.dc.html`), correct Content-Type, `Cache-Control: private, max-age=3600`. Mirrors `api/media/[id]`. 17 tests. |
