...
/**
 * STUB — minimal working "playful" surprise (magic 8-ball energy). D5 replaces
 * with the real bonus tool. Keep the default export shape.
 */
function BonusStub({ options, onResult }: SelectorGameProps) {
  ...
  function run() {
    ...
    const result: DecideResult = {
      ...
      detail: { tool: "bonus", placeholder: true },
    }
    onResult(result)
  }
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-2 text-center">
      ...
      >
        Placeholder · D5 builds the real bonus tool
      </span>
      ...
      <motion.button
        ...
        style={{ background: "var(--color-amber)", color: "#2A2018", fontFamily: "var(--font-nav)" }}
        data-testid="decide-game-run"
      >
        {picked ? "Ask again" : "Ask fate"}
      </motion.button>
    </div>
  )
}
...
const bonus: SelectorGame = {
  id: "bonus",
  label: "Ask Fate",
  arabicLabel: "الكرة السحرية",
  whenToUse: "Can't choose? Leave it to the universe — magic 8-ball energy.",
  kind: "playful",
